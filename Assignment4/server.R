...
shinyServer(function(input, output, session) {
  ...
  getData <- reactive({
    data <- read.csv(file="D:/DataScienceinindustry/Assignment4/Ass4Data.csv")
  ...
  getXGBModels<-reactive({
    ...
    mods<- caret::train(Y ~ ., 
                        ...
                        method = method,
                        trControl = trControl,
                        tuneGrid = expand.grid(.nrounds = seq(1,1,1), 
  ...
  output$XGBSummary1 <- renderTable({
    mods <- getXGBModels()
    as.data.frame(mods$bestTune)
  })  
  
  output$XGBPlots <- renderPlot({
    plot(getXGBModels())
  })     
  
  output$XGBSummary2 <- renderPrint({
    mods <- getXGBModels()
    print(mods$finalModel)
  })
  ...
  getSVMModels<-reactive({
    method <- "svmRadial" 
    showNotification(id = method, paste("Optimising", method, "hyper-parameters using cross validation"),  session = session, duration = NULL)
    mods=caret::train(Y~.,data=getTrainData(),method=method,trControl=trControl,
                      tuneGrid = expand.grid(.sigma = seq(.01, 0.5 , by = 0.05),
                                             .C = seq(0.75, 3, by = 0.5)),
    allowParallel = TRUE
  )
    removeNotification(id=method)
  ...
  getRPModels<-reactive({
    method <- "rpart2" 
    showNotification(id = method, paste("Optimising", method, "hyper-parameters using cross validation"),  session = session, duration = NULL)
    
    mods<-caret::train(Y ~ . , data = getTrainData(), method = method,
                       trControl = trControl,
                       tuneGrid=expand.grid(.maxdepth = seq(1,20,1))
   
                       #Allow Parrallelism doesnt work with rpart2. 
    )
    removeNotification(id=method)
    mods
  ...
  output$RPartSummary1 <- renderTable({
    mods <- getRPModels()
    as.data.frame(mods$bestTune)
  })  
  
  output$RPartPlots <- renderPlot({
    plot(getRPModels())
  })     
  
  output$RPartSummary2 <- renderPrint({
    mods <- getRPModels()
    print(mods$finalModel)
  })
  ...
  getAllModels <- reactive({
    list(GLMnet=getGlmModels(), PLS=getPlsModels(), ANN=getAnnModels(),XGB=getXGBModels(),SVM=getSVMModels(),RP=getRPModels()) # expand this list with further models
  })
